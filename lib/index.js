...
class RableAuth {
    ...
    constructor(input) {
        ...
        this.authAPI.registerAction('login', (req, res) => {
            if (req.body.identifier !== undefined && req.body.password !== undefined) {
                const user = this.#callbacks.obtainUser(req.body.identifier);
                if (user == null) {
                    res.json({
                        success: false,
                        error: 'unknown_user',
                    ...
                    if (user[this.config.systemIdentifier] === undefined || user.password === undefined) {
                        ...
                        bcrypt.compare(req.body.password, user.password, (err, correctPassword) => {
